...
def _project(tmp_path: Path, version: str = "0.1.0") -> Path:
    path = tmp_path / "pyproject.toml"
    path.write_text(f'[project]\nversion = "{version}"\n', encoding="utf-8")
    return path


def test_current_stable_tag_matches_exactly(tmp_path: Path) -> None:
    assert version_from_tag("v0.1.0", _project(tmp_path)) == "0.1.0"


@pytest.mark.parametrize(
    "tag",
    ["v0.1.0dev0", "v0.1.0.dev0", "0.1.0", "v0.1.0/../../main", "v0.1.0 dev0"],
)
